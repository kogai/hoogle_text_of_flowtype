...
let rec declarations ?(root="") (_, statements, comments) =
  let rec translate_statement = Statement.(function
      | _, DeclareFunction ({
      ...
      | _, TypeAlias {
        ...
        |> (fun (loc, base) -> (loc, (type_of_type_var base)))
        |> Option.return
      | loc, DeclareModule { DeclareModule.id; body = (_, { Block.body = xs; }); } ->
        let id = match id with
          | DeclareModule.Identifier (_, id) -> id
        ...
        in
        let declares = declarations ~root (loc, xs, comments) in
        let module_name = "-- |\n" ^ "module " ^ id ^ "\n" in
        Some (loc, String.concat ~sep:"\n" @@ module_name::declares)
      | _, ImportDeclaration _ -> None
      | _, _ ->
        ignore @@ Utils.unreachable ~message:"Hit unhandled match";
        None
    )
